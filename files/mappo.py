    ...
    def get_batch(self, batch_size: int) -> Dict:
        ...
        for key in self.states.keys():
            if len(self.states[key]) > 0:
                data[f'{key}_states'] = torch.stack(self.states[key])
...
class MAPPO:
    ...
    def _init_networks(self):
        ...
        retailer_state_dim = self.config.num_products * 4 + 1  # inventory + demand encoding + time
        warehouse_state_dim = self.config.num_products * 4 + 4  # inventory + demand + costs + time
        global_state_dim = (retailer_state_dim * self.config.num_retailers + 
                           warehouse_state_dim + 
                           self.config.num_products * (self.config.num_retailers + 2) + 1)
    ...
    def select_actions(self, states: Dict, deterministic: bool = False) -> Dict:
        ...
        actions['warehouse'] = warehouse_action.cpu().numpy().squeeze()
        ...
        if 'predictor' in states:
            ...
            actions['predicted_demand'] = pred_action.cpu().numpy().squeeze()
    ...
    def _update_actors(self, batch: Dict) -> float:
        ...
        if 'retailers_states' in batch:
            ...
            for i, actor in enumerate(self.retailer_actors):
                ...
                states_i = retailer_states[:, i]
                actions_i = retailer_actions[:, i]
        ...
        if 'warehouse_states' in batch:
            warehouse_states = batch['warehouse_states']
            warehouse_actions = batch['warehouse_actions']
            warehouse_log_probs_old = batch['warehouse_log_probs']
            warehouse_advantages = batch['warehouse_advantages']
